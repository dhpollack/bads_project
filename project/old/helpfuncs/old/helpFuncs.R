...
helpFuncs.load.dataset <- function(fps = "./Initial_2_Datasets/assignment_BADS_WS1617_known.csv")
  ...
  tmp_ad_levs[which(tmp_ad_levs=="")] = -1
  ...
  tmp_weight_regressors = c('used_items', 'book_count', 'paperback_count', 'schoolbook_count', 'film_count', 'musical_count', 'imported_count', 'other_count')
  ...
  # binning item_count
  cutoffs <- c((-1),0,1,2,4,7,12,20,max(df1$item_count))
  bin.item.count <- cut(df1$item_count, cutoffs)
  df1$x_item_count_binned <- bin.item.count
  
  # Binning book_count
  # Bin remitted items
  # I know from Aleksandra's analysis that the maximum value for remitted
  # is 15
  
  # This might be useful later on bacause we need to modify both datasets ;)
  cap <- max(df1$book_count)
  cutoffs <- c((-1),0,1,2,4,7,cap)
  bin.book_count <- cut(df1$book_count, cutoffs)
  
  # binning remitted_items
  cutoffs <- c((-1),0,1,2,3,5,10,max(df1$remitted_items))
  bin.remitted_items <- cut(df1$remitted_items, cutoffs)

  # variable for the combination of coupon and return_customer
  #f("return_customer" %in% colnames(df1)) {
  #  tmp_df1_coup_ret = sapply(df1[,c("coupon", "return_customer")],as.numeric)
  #  tmp_df1_coup_ret[, 1] = tmp_df1_coup_ret[, 1] - 1 # make coupon go from 0 to 1
  #  tmp_cr_grid = expand.grid(coupon = unique(tmp_df1_coup_ret[, 1]), return_customer = unique(tmp_df1_coup_ret[, 2]))
  #  df1$x_cr_grid = apply(tmp_df1_coup_ret,
  #                        1,
  #                        function(x) { which(tmp_cr_grid$coupon == x[1] & tmp_cr_grid$return_customer == x[2])})
  #}
  ##########
  # Purge
  ##########
