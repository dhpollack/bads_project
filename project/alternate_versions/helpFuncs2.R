...
helpFuncs.load.dataset <- function(fps = "./Initial_2_Datasets/assignment_BADS_WS1617_known.csv")
  ...
  # binning item_count
  cutoffs <- c((-1),0,1,2,4,7,12,20,Inf)
  ...
  # cap <- max(max(df1$book_count), max(df2$book_count))
  cutoffs <- c((-1),0,1,2,4,7,Inf)
  bin.book_count <- cut(df1$book_count, cutoffs)
  head(bin.book_count)
  head(df1$book_count)
  
  # binning remitted_items
  cutoffs <- c((-1),0,1,2,3,5,10,Inf)
  bin.remitted_items <- cut(df1$remitted_items, cutoffs)
  head(bin.remitted_items)
  head(df1$remitted_items)
