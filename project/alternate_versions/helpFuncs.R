...
function(fps = "./data/train.csv", outputPath = "./output/", epochDate = "2013/1/1") {
  ...
  levels(df1$postcode_delivery)[which(levels(df1$postcode_delivery)=="")] = floor(runif(1, min = 1, max = 100))
  levels(df1$postcode_delivery)[which(levels(df1$postcode_delivery)=="0")] = floor(runif(1, min = 1, max = 100))
  if("00" %in% levels(df1$postcode_delivery)) {
    levels(df1$postcode_delivery)[which(levels(df1$postcode_delivery)=="00")] = floor(runif(1, min = 1, max = 100))
  }
  levels(df1$postcode_delivery)[!grepl("[0-9]+",levels(df1$postcode_delivery))] = floor(runif(1, min = 1, max = 100))
  levels(df1$postcode_delivery) = as.numeric(levels(df1$postcode_delivery))
